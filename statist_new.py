...
def monger(league, seas_type, spot, team, limit):
    query = {'league': league, 'seas_type': 'season'}
    if spot == 'ever':
    ...
    return db.matches.find(query).sort([('date.iso', -1)]).limit(limit)


pool = mp.Pool(processes=4)
resp = [
    pool.apply(monger, args=('1-6', xeid, xhash, dict(ftot='3-1', frst='3-3'))),
    pool.apply(monger, args=('1-6', xeid, xhash, dict(ftot='5-1', frst='5-3'))),
    pool.apply(monger, args=('1-6', xeid, xhash, dict(ftot='2-1', frst='2-3'))),
    pool.apply(monger, args=('1-6', xeid, xhash, dict(ftot='2-1', frst='2-3')))
]
pool.close()
